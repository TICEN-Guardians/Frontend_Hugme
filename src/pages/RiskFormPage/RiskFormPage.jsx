...
import { analyzeDiagnosis, createDiagnosis, resolveProperty, searchProperty, uploadRegistry } from '../../services/propertyRisk/propertyRiskService.js';
...
export default function RiskFormPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    let currentStep = '물건 정보를 확인';
    try {
      setProgressMessage('물건 정보를 확인하고 있습니다.');
      await resolveProperty({ address: address.trim(), dongName: selectedCandidate.dongName, hoName: unitNumberRequired ? hoName.trim() : null });
      currentStep = '진단 요청을 생성';
      setProgressMessage('진단 요청을 생성하고 있습니다.');
      const diagnosis = await createDiagnosis({
        address: address.trim(), dongName: selectedCandidate.dongName || null,
        hoName: unitNumberRequired ? hoName.trim() : null,
        deposit: Number(deposit.replaceAll(',', '')), contractDate,
        contractArea: contractAreaRequired ? Number(area) : null,
        exclusiveArea: contractAreaRequired ? null : Number(area),
        floor: Number(floor), landlordName: landlordName.trim(),
      });
      currentStep = '등기부등본을 분석';
      setProgressMessage('등기부등본을 분석하고 있습니다.');
      await uploadRegistry({ analysisId: diagnosis.analysisId, file });
      currentStep = '시세와 위험도를 계산';
      setProgressMessage('시세와 위험도를 계산하고 있습니다.');
      await analyzeDiagnosis(diagnosis.analysisId);
      navigate(`/risk/${diagnosis.analysisId}`);
    } catch (error) {
  ...
  return (
    <div className={styles.root}>
      <form className={styles.form} onSubmit={handleSubmit} noValidate>
        ...
        <motion.div className={styles.card} initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: prefersReducedMotion ? 0 : 0.14 } } }}>
          <motion.div className={styles.primaryColumn} variants={motionVariants}>
            <div className={styles.field}>
              ...
              <div className={styles.inlineRow}>
                <input id="risk-address" className={styles.input} placeholder="도로명, 지번, 건물명 등으로 검색하세요" value={address} onChange={(event) => { setAddress(event.target.value); resetAddressResult(); }} />
                <button type="button" className={styles.searchButton} disabled={isSearching} onClick={handleAddressSearch}>{isSearching ? '검색 중' : '주소 검색'}</button>
              </div>
              {errors.address && <p className={styles.fieldError}>{errors.address}</p>}
              {candidates.length > 0 && <select className={styles.input} value={selectedIndex} onChange={(event) => setSelectedIndex(event.target.value)}>
            ...
            {unitNumberRequired && <TextField id="risk-ho" label="호수" value={hoName} setValue={setHoName} error={errors.hoName} placeholder="예: 402" />}
            <div className={styles.twoColumnFields}>
              <UnitField id="risk-area" label={contractAreaRequired ? '계약 면적' : '전용 면적'} unit="m²" value={area} setValue={setArea} error={errors.area} inputMode="decimal" placeholder="예: 33.12" />
              <UnitField id="risk-floor" label="해당 층" unit="층" value={floor} setValue={setFloor} error={errors.floor} inputMode="numeric" placeholder="예: 13" />
            </div>
            <TextField id="risk-landlord" label="계약 상대방 이름" value={landlordName} setValue={setLandlordName} error={errors.landlordName} placeholder="등기부 소유자와 비교할 임대인 이름" />
            <UnitField id="risk-deposit" label="전세보증금" unit="원" value={deposit} setValue={(value) => setDeposit(value.replace(/[^0-9,]/g, ''))} error={errors.deposit} inputMode="numeric" placeholder="예: 200000000" />
            <div className={styles.field}><label className={styles.label} htmlFor="risk-contract-date">계약 예정일 <span className={styles.required}>•</span></label><div className={styles.unitInputWrapper} onClick={openContractDatePicker}><input ref={contractDateInputRef} id="risk-contract-date" type="date" className={styles.unitInput} value={contractDate} onChange={(event) => setContractDate(event.target.value)} /></div>{errors.contractDate && <p className={styles.fieldError}>{errors.contractDate}</p>}</div>
          </motion.div>
          ...
          <motion.div className={styles.sideColumn} variants={motionVariants}>
            <div className={styles.field}>
              ...
              <div className={styles.dropzone}><div className={styles.dropzoneInfo}><span className={styles.uploadIcon}><FaArrowUp /></span><div><p className={styles.dropzoneTitle}>{file?.name ?? 'PDF 파일 업로드'}</p><p className={styles.dropzoneDescription}>PDF · 최대 10MB · 최근 1개월 이내 발급본 권장</p></div></div><input ref={fileInputRef} type="file" accept={ACCEPTED_TYPE} onChange={handleFileChange} className={styles.hiddenInput} /><button type="button" className={styles.fileSelectButton} onClick={() => fileInputRef.current?.click()}>파일 선택</button></div>
        ...
        <motion.div className={styles.actionRow} initial={{ opacity: 0 }} animate={{ opacity: 1 }}><Button type="submit" disabled={isSubmitting} className={styles.submitButton}>{isSubmitting ? '진단 진행 중...' : '위험도 진단 시작하기'}</Button></motion.div>
      </form>
...
function TextField({ id, label, value, setValue, error, placeholder }) {
  return <div className={styles.field}><label className={styles.label} htmlFor={id}>{label} <span className={styles.required}>•</span></label><input id={id} className={styles.input} placeholder={placeholder} value={value} onChange={(event) => setValue(event.target.value)} />{error && <p className={styles.fieldError}>{error}</p>}</div>;
}

function UnitField({ id, label, unit, value, setValue, error, inputMode, placeholder }) {
  return <div className={styles.field}><label className={styles.label} htmlFor={id}>{label} <span className={styles.required}>•</span></label><div className={styles.unitInputWrapper}><input id={id} className={styles.unitInput} inputMode={inputMode} placeholder={placeholder} value={value} onChange={(event) => setValue(event.target.value)} /><span className={styles.unit}>{unit}</span></div>{error && <p className={styles.fieldError}>{error}</p>}</div>;
}
